...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	if (bus->vars) {
		...
		if (!nvram_ularray) {
		    MFREE(bus->dhd->osh, vbuffer, varsize);  //CONN-EC-Coverity39548-01+
			return BCME_NOMEM;
        }
...
dhd_bus_console_in(dhd_pub_t *dhdp, uchar *msg, uint msglen)
{
	...
	if ((pkt = PKTGET(bus->dhd->osh, 4 + SDPCM_RESERVE, TRUE)) != NULL)
		rv = dhdsdio_txpkt(bus, pkt, SDPCM_EVENT_CHANNEL, TRUE);  //CONN-EC-Coverity39527-01*
...
dhd_bus_select_firmware_name_by_chip(struct dhd_bus *bus, char *dst, char *src)
{
	...
	strncpy(dst, src, MOD_PARAM_PATHLEN);  //MTD-CONN-CD-WIFI-COVERITY_63405-01
	...
	while (i>0){
        ...
        case BCM4330_CHIP_ID:
                if (bus->sih->chiprev==BCM4330B1_CHIP_REV)
                        strncpy(&dst[i+1], bcm4330b1_fw_name[fw_type], MOD_PARAM_PATHLEN); //MTD-CONN-CD-WIFI-COVERITY_63405-00*
                else
                        strncpy(&dst[i+1], bcm4330b2_fw_name[fw_type], MOD_PARAM_PATHLEN); //MTD-CONN-CD-WIFI-COVERITY_63405-00*
                break;
        case BCM43362_CHIP_ID:
                if (bus->sih->chiprev==BCM43362A0_CHIP_REV)
                        strncpy(&dst[i+1], bcm43362a0_fw_name[fw_type], MOD_PARAM_PATHLEN); //MTD-CONN-CD-WIFI-COVERITY_63405-00*
                else
                        strncpy(&dst[i+1], bcm43362a2_fw_name[fw_type], MOD_PARAM_PATHLEN); //MTD-CONN-CD-WIFI-COVERITY_63405-00*
                break;
        }
}
...
//	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
	while ((bcmerror = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {  //CONN-EC-Coverity39537-01*
//CONN-EC-Coverity39537-01+[
        if (bcmerror <= 0) {
			DHD_ERROR(("%s: error %d on dhd_os_get_image_block\n",
