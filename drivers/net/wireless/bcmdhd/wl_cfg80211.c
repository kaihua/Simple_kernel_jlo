	...
	struct vif_params *params)
{
	...
	if (wl->p2p_supported && (wlif_type != -1)) {
		...
		//MTD-CONN-CD-WIFI-COVERITY_63353-01*[
		if (!wl->p2p)
		{
			WL_ERR(("wl->p2p is NULL.\n"));
			goto fail;
		}
		//MTD-CONN-CD-WIFI-COVERITY_63353-01*]
...
wl_set_auth_type(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	case NL80211_AUTHTYPE_NETWORK_EAP:
		val = WL_AUTH_OPEN_SHARED; //MTD-CONN-CD-WIFI-COVERITY_63357-00+
		WL_DBG(("network eap\n"));
		break; //MTD-CONN-CD-WIFI-COVERITY_63357-00+
	default:
		val = WL_AUTH_OPEN_SHARED;
	...
	u8 key_idx, const u8 *mac_addr, struct key_params *params)
{
	...
	if (key.len == 0) {
		/* key delete */
		swap_key_from_BE(&key);
		err = wldev_iovar_setbuf_bsscfg(dev, "wsec_key", &key, sizeof(key), //MTD-CONN-CD-WIFI-COVERITY_63347-00*
			wl->ioctl_buf, WLC_IOCTL_MAXLEN, bssidx, &wl->ioctl_buf_sync);
		if (unlikely(err)) {
			WL_ERR(("key delete error (%d)\n", err));
			//return err; //MTD-CONN-CD-WIFI-COVERITY_63347-00-
		}
	} else {
	...
	u8 key_idx, bool pairwise, const u8 *mac_addr)
{
	...
	/* Set the new key/index */
	swap_key_from_BE(&key);
	err = wldev_iovar_setbuf_bsscfg(dev, "wsec_key", &key, sizeof(key), wl->ioctl_buf, //MTD-CONN-CD-WIFI-COVERITY_63348-00*
		WLC_IOCTL_MAXLEN, bssidx, &wl->ioctl_buf_sync);
	if (unlikely(err)) {
		if (err == -EINVAL) {
		...
		//return err; //MTD-CONN-CD-WIFI-COVERITY_63348-00-
	}
	return err;
}
	...
	void (*callback) (void *cookie, struct key_params * params))
{
	...
	err = wldev_iovar_getint_bsscfg(dev, "wsec", &wsec, bssidx); //MTD-CONN-CD-WIFI-COVERITY_63349_63335-00*
	if (unlikely(err)) {
		WL_ERR(("WLC_GET_WSEC error (%d)\n", err));
		//return err; //MTD-CONN-CD-WIFI-COVERITY_63349_63335-00-
	}
	...
	struct cfg80211_pmksa *pmksa)
{
	...
	CHECK_SYS_UP(wl);
	memset(&pmkid, 0, sizeof(struct _pmkid_list));  //MTD-CONN-CD-WIFI-COVERITY_63425-00+
	...
	u64 *cookie)
{
	...
	*cookie = id;
	...
	if (wl_cfgp2p_is_pub_action(action_frame->data, action_frame->len)) {
		...
		if (act_frm && act_frm->subtype == P2P_PAF_GON_REQ) { //MTD-CONN-CD-WIFI-COVERITY_63395-00*
			WL_DBG(("P2P: GO_NEG_PHASE status set \n"));
			wl_set_p2p_status(wl, GO_NEG_PHASE);
		} else if (act_frm && act_frm->subtype == P2P_PAF_GON_CONF) { //MTD-CONN-CD-WIFI-COVERITY_63395-00*
		...
		if (act_frm && act_frm->subtype == P2P_PAF_GON_RSP) //MTD-CONN-CD-WIFI-COVERITY_63395-00*
			retry_cnt = 1;
...
wl_validate_wpa2ie(struct net_device *dev, bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	//MTD-CONN-CD-WIFI-COVERITY_63381-00*[
	//tmp = mcast->oui;
	tmp = &mcast->type;
	...
	//MTD-CONN-CD-WIFI-COVERITY_68740-00*[
	//tmp = ucast->list[0].oui;
	//switch (tmp[DOT11_OUI_LEN]) {
	tmp = &ucast->list[0].type;
	switch (*tmp) {
	//MTD-CONN-CD-WIFI-COVERITY_68740-00*]
		case WPA_CIPHER_NONE:
			pval = 0;
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		wldev_iovar_getbuf_bsscfg(dev, "cur_etheraddr",
			NULL, 0, wl->ioctl_buf, WLC_IOCTL_MAXLEN, bsscfgidx, &wl->ioctl_buf_sync);
		//MTD-CONN-CD-WIFI-COVERITY_63336-00*[
		err = wldev_ioctl(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN, false);
		if (unlikely(err)) {
			WL_ERR(("get bssid error (%d)\n", err));
		}
		//MTD-CONN-CD-WIFI-COVERITY_63336-00*]
		memcpy(da.octet, wl->ioctl_buf, ETHER_ADDR_LEN);
...
{
	struct wl_priv *wl = wlcfg_drv_priv;
	struct wireless_dev *wdev = NULL; //= wl->p2p_wdev; //MTD-CONN-CD-WIFI-COVERITY_63394-00*

	//MTD-CONN-CD-WIFI-COVERITY_63394-00+[
	if(wl)
		wdev = wl->p2p_wdev;
	//MTD-CONN-CD-WIFI-COVERITY_63394-00+]
	WL_DBG(("Enter \n"));
