...
//		strcpy(ifr.ifr_name, dev->name);
		strncpy(ifr.ifr_name, dev->name, IFNAMSIZ - 1);  //CONN-EC-Coverity39563-01*
        ifr.ifr_name[IFNAMSIZ - 1] = '\0';  //CONN-EC-Coverity39563-01*
...
//	strcpy(buf, "cur_etheraddr");
	strncpy(buf, "cur_etheraddr", sizeof(buf));  //CONN-EC-Coverity39567-01*
    buf[sizeof(buf) - 1] = '\0';  //CONN-EC-Coverity39567-01+
...
//			p += sprintf(p, "\\x%02X", c);
			p += snprintf(p, 4, "\\x%02X", c);  //CONN-EC-Coverity39564-01*
		}
	}
	*p = '\0';
...
//			strcpy(iovbuf, "bcn_li_dtim");
			strncpy(iovbuf, "bcn_li_dtim", sizeof(iovbuf) - 1); //CONN-EC-Coverity39566-01*
...
//	strcpy(extra, flag);
	strncpy(extra, flag, IW_CUSTOM_MAX); //CONN-EC-Coverity39570-01*
    extra[IW_CUSTOM_MAX] = '\0';  //CONN-EC-Coverity39570-01+
...
//	strcpy(cwrq, "IEEE 802.11-DS");
	strncpy(cwrq, "IEEE 802.11-DS", strlen("IEEE 802.11-DS") + 1); //CONN-EC-Coverity39568-01*
...
//	for (i = 0; i < rateset.count && i < IW_MAX_BITRATES; i++)
	for (i = 0; i < rateset.count && i < 16; i++)  //CONN-EC-Coverity63382-01*
	...
	while (p_buf != iscan->list_cur) {
		...
		for (ii = 0; ii < list->count && apcnt < IW_MAX_AP; apcnt++, ii++) {
			...
			if (bi->rateset.count) {
				...
				iwe.u.bitrate.fixed = iwe.u.bitrate.disabled = 0;
//				for (j = 0; j < bi->rateset.count && j < IW_MAX_BITRATES; j++) {
				for (j = 0; j < bi->rateset.count && j < 16; j++) {  //CONN-EC-Coverity63384-01*
					iwe.u.bitrate.value =
					        (bi->rateset.rates[j] & 0x7f) * 500000;
					value = IWE_STREAM_ADD_VALUE(info, event, value, end, &iwe,
					                             IW_EV_PARAM_LEN);
...
//	strcpy(extra, iw->nickname);
	strncpy(extra, iw->nickname, IW_ESSID_MAX_SIZE); //CONN-EC-Coverity39569-01*
    extra[IW_ESSID_MAX_SIZE] = '\0';  //CONN-EC-Coverity39569-01+
...
//CONN-EC-Coverity39571-01*				sprintf(ptr, "%02x%02x%02x%02x", (uint)output[i*4],
				snprintf(ptr, (WSEC_MAX_PSK_LEN - (i * 8)), "%02x%02x%02x%02x", (uint)output[i*4],
...
//CONN-EC-Coverity39571-02*				sprintf(ptr, "%02x%02x%02x%02x", (uint)output[i*4],
				snprintf(ptr, (WSEC_MAX_PSK_LEN - (i * 8)), "%02x%02x%02x%02x", (uint)output[i*4],
					(uint)output[i*4+1], (uint)output[i*4+2],
				        (uint)output[i*4+3]);
				ptr += 8;
	...
	char *ext)
{
	...
	if ((wrqu->data.length > 4) && (wrqu->data.length < sizeof(extra))) {
		...
		extra[255] = '\0';  //CONN-EC-Coverity39575-01+
...
//	if (!(extra = kmalloc(dwrq->length, GFP_KERNEL)))
	if (!(extra = kmalloc(dwrq->length + 1, GFP_KERNEL)))  //CONN-EC-Coverity39576-01*
	    return -ENOMEM;
    ...
    extra[dwrq->length] = '\0';  //CONN-EC-Coverity39576-01+
		...
		if (ap_cfg_running) {
			...
			if (ap_macmode)
			{
				...
				if (i == ap_black_list.count) {
					...
					char mac_buf[32] = {0};
//CONN-EC-Coverity39565-02*					sprintf(mac_buf, "STA_BLOCK %02X:%02X:%02X:%02X:%02X:%02X",
					snprintf(mac_buf, sizeof(mac_buf), "STA_BLOCK %02X:%02X:%02X:%02X:%02X:%02X",
...
//		strcpy(extra, PNO_EVENT_UP);
		strncpy(extra, PNO_EVENT_UP, sizeof(extra) - 1); //CONN-EC-Coverity39565-01*
		wrqu.data.length = strlen(extra);
	}
	break;
