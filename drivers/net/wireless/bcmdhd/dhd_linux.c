...
#else
#define COPY_FW_PATH_BY_CHIP( bus, dst, src)	strncpy(dst, src, MOD_PARAM_PATHLEN - 1)   //MTD-Connectivity-FY-Coverity_39560_01
#endif
...
//MTD_CONN_EC_Tapioca_ICS-00516-	int power_mode = PM_FAST;
    ...
    if (dhd) {   //CONN-EC-Coverity63352-01+

	dhd_suspend_lock(dhd);
//	if (dhd && dhd->up) {
	if (dhd->up) {  //CONN-EC-Coverity63352-01*
...
_dhd_set_multicast_list(dhd_info_t *dhd, int ifidx)
{
	...
	strncpy(bufp, "mcast_list", buflen);  //MTD-Connectivity-FY-Coverity_39556
...
dhd_toe_get(dhd_info_t *dhd, int ifidx, uint32 *toe_ol)
{
	...
	strncpy(buf, "toe_ol", sizeof(buf));  //MTD-Connectivity-FY-Coverity_39560
...
dhd_toe_set(dhd_info_t *dhd, int ifidx, uint32 toe_ol)
{
	...
	strncpy(buf, "toe_ol", sizeof(buf));  //MTD-Connectivity-FY-Coverity_39562
	...
	strncpy(buf, "toe", sizeof(buf));    //MTD-Connectivity-FY-Coverity_39562
...
dhd_ethtool_get_drvinfo(struct net_device *net, struct ethtool_drvinfo *info)
{
	...
	snprintf(info->driver, sizeof(info->driver), "wl");    //MTD-Connectivity-FY-Coverity_39559
	snprintf(info->version, sizeof(info->version),"%lu", dhd->pub.drv_version);  //MTD-Connectivity-FY-Coverity_39559
}
...
dhd_ethtool(dhd_info_t *dhd, void *uaddr)
{
	...
	case ETHTOOL_GDRVINFO:
		...
		if (strcmp(drvname, "?dhd") == 0) {
			snprintf(info.driver, sizeof(info.driver), "dhd");  //MTD-Connectivity-Coverity_39558
			strncpy(info.version, EPI_VERSION_STR, sizeof(info.version));  //MTD-Connectivity-Coverity_39558
		}
		...
		/* finally, report dongle driver type */
			...
			snprintf(info.driver, sizeof(info.driver),"wl");   //MTD-Connectivity-Coverity_39558
		else
			snprintf(info.driver, sizeof(info.driver),"xx");   //MTD-Connectivity-Coverity_39558

		snprintf(info.version, sizeof(info.version), "%lu", dhd->pub.drv_version);   //MTD-Connectivity-Coverity_39558
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	//MTD-Connectivity-FY-Coverity_63433++
	if (!access_ok( VERIFY_READ, ifr->ifr_data, sizeof(wl_ioctl_t))) {  //MTD-Connectivity-FY-Coverity_63433_01
...
//	if ((firmware_path != NULL) && (firmware_path[0] != '\0')) {
	if (firmware_path[0] != '\0') {  //CONN-EC-Coverity39542-01*
...
//	if ((firmware_path != NULL) && (firmware_path[0] != '\0'))
	if (firmware_path[0] != '\0') //CONN-EC-Coverity39540-01*
		COPY_FW_PATH_BY_CHIP(bus, fw_path, firmware_path);
	
//	if ((nvram_path != NULL) && (nvram_path[0] != '\0'))
	if (nvram_path[0] != '\0') //CONN-EC-Coverity39540-01*
		strncpy(nv_path, nvram_path, sizeof(nv_path) - 1);  //MTD-Connevtivity-FY-Coverity_39557_01
	...
	if (iface_name[0]) {
		...
		if ((ch > '9' || ch < '0') && (len < IFNAMSIZ - 2))
			strncat(net->name, "%d", (IFNAMSIZ - len)); //MTD-Connectivity-FY-Coverity_39557
	}
...
//		(nv_path != NULL) && (nv_path[0] != '\0')) {
		(fw_path[0] != '\0') && (nv_path[0] != '\0')) {  //CONN-EC-Coverity39541-01*
...
#endif
		{
//CONN-EC-Coverity39552-01-			if (ifp->net) {
				unregister_netdev(ifp->net);
				free_netdev(ifp->net);
				ifp->net = NULL;
//			}
...
dhd_module_init(void)
{
    ...
    // Pull power in msm7x2x_init() to avoid mmc driver failed to detect Wi-Fi chip.
	//CONN-EC-WIFI-Porting-02- dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);
...
{
	//MTD-CONN-CD-WIFI-COVERITY_39532_63351-00*[
	dhd_info_t * dhd = NULL;// = (dhd_info_t *)(pub->info);
