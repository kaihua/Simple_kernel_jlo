/* drivers/input/sensor/akm8975.c - akm8975 compass driver
 *
...
AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char i2c_rbuf[RWBUF_SIZE] = {0};	/* for READ */
	char i2c_wbuf[RWBUF_SIZE] = {0};	/* for WRITE */
	...
	case ECS_IOCTL_READ:
		...
		if (copy_from_user(&i2c_rbuf, argp, sizeof(i2c_rbuf))) {
			dev_err(&akm->i2c->dev, "copy_from_user read failed.");
			return -EFAULT;
		}
		break;
	case ECS_IOCTL_WRITE:
		...
		if (copy_from_user(&i2c_wbuf, argp, sizeof(i2c_wbuf))) {
			dev_err(&akm->i2c->dev, "copy_from_user write failed.");
			return -EFAULT;
		}
		break;
	...
	case ECS_IOCTL_READ:
		...
		if ((i2c_rbuf[0] < 1) || (i2c_rbuf[0] > (RWBUF_SIZE-1))) {
			dev_err(&akm->i2c->dev, "invalid argument.");
			return -EINVAL;
		}
		ret = akm8975_i2c_rxdata(akm->i2c, &i2c_rbuf[1], i2c_rbuf[0]);
		if (ret < 0)
			return ret;
		break;
	case ECS_IOCTL_WRITE:
		dev_vdbg(&akm->i2c->dev, "IOCTL_WRITE called.");
		if ((i2c_wbuf[0] < 2) || (i2c_wbuf[0] > (RWBUF_SIZE-1))) {
			dev_err(&akm->i2c->dev, "invalid argument.");
			return -EINVAL;
		}
		ret = akm8975_i2c_txdata(akm->i2c, &i2c_wbuf[1], i2c_wbuf[0]);
		if (ret < 0)
			return ret;
		break;
	case ECS_IOCTL_SET_MODE:
	...
	switch (cmd) {
	case ECS_IOCTL_READ:
		if (copy_to_user(argp, &i2c_rbuf, i2c_rbuf[0]+1)) {
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		pdata->gpio_init();	/* FIH-SW2-PERIPHERAL-FG-MSENSOR-00+ */
	} else {
	...
	/***** IRQ setup *****/
	client->irq = MSM_GPIO_TO_INT(GPIO_COMPASS_INT);	/* FIH-SW2-PERIPHERAL-FG-MSENSOR-00+ */
